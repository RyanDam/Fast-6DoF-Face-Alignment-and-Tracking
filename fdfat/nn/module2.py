...
class LightWeightBackbone(nn.Module):
    ...
    def __init__(self, expand_rate=2, muliplier=1, act=True):
        ...
        self.conv22 = IdentifyBlock(int(16*muliplier), int(32*muliplier), expand_rate, act=self.act)
...
class MainStreamModule(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv11(x)
        # size / 8
        stage_1 = self.conv21(x)

        x = self.conv22(stage_1)
        stage_2 = self.conv23(x)

        x1 = self.convp1(ori)
        # size / 8
        x1f = self.global_pool(x1).flatten(start_dim=1)

        x2 = self.convp2(stage_1)
        # size / 16
        x2f = self.global_pool(x2).flatten(start_dim=1)

        x3 = self.convp3(stage_2)
        # size / 16
